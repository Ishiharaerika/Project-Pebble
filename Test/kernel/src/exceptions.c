...
int handle_create(SceUID pid, SceProcEventInvokeParam2 *a2, int a3)
{
    (void)a2;
    (void)a3;
    kernel_debugger_attach(pid);
    return 0;
}

int handle_exit(SceUID pid, SceProcEventInvokeParam1 *a2, int a3)
{
    (void)pid;
    (void)a2;
    (void)a3;
...
{
    // if(!isDebugging) return EXCEPTION_NOT_HANDLED;
    ...
    // ksceKernelSetEventFlag(g_breakpoint_triggered, 0x1); // || ksceKernelWaitEventFlag(g_breakpoint_triggered, 0x1,
    // SCE_EVENT_WAITAND | SCE_EVENT_WAITCLEAR, NULL, NULL) >= 0
...
int register_handler(void)
{
    ...
    // g_breakpoint_triggered = ksceKernelCreateEventFlag("BKPTTrigger", SCE_KERNEL_ATTR_THREAD_FIFO, 0, NULL);

    if (ksceKernelRegisterProcEventHandler("pebbleBKPT", &handler, 0) < 0)
        return ksceKernelPrintf("Failed registering EventHandler.");
