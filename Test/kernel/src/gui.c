...
static void draw_ui(void)
{
    ...
    case UI_WELCOME: {
        ...
        if (g_target_process.pid > 0)
        {
            ...
            rendererv_drawStringF(100, 200, "Press %x to continue to HEX view...", guistate.hotkeys.confirm);
        }
        break;
    }
...
int pebble_thread(SceSize args, void *argp)
{
    ...
    while (1)
    {
        ...
        if (released == guistate.hotkeys.show_gui || (guistate.gui_visible && guistate.ui_state == UI_WELCOME && (released & guistate.hotkeys.cancel)))
        {
            guistate.gui_visible = !guistate.gui_visible;
            if (guistate.gui_visible == false) 
                rendererv_freeVirtualFB();
            else 
                rendererv_allocVirtualFB();
        }
        
        if (guistate.gui_visible && pebble_mtx_uid > 0)
        {
