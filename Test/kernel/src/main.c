...
{
    if ( // module_get_export_func(0x10005, "SceKernelThreadMgr", THREADMGR_NID, SETTHBP_NID, (uintptr_t
         // *)&ksceKernelSetTHBP) < 0 ||
    ...
    SceUID thid = ksceKernelCreateThread("pebble", pebble_thread, 0x40, 0x20000, 0, 0, NULL);
    if (thid >= 0)
