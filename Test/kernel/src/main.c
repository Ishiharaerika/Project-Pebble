...
int kernel_set_watchpoint(uint32_t address, WatchPointBreakType type) {
    ...
    if (ksceKernelSetPHWP(g_target_process.pid, index, (void *)address, WCR) >= 0) {
        ...
        slot->address = address;
        slot->index = index;
        slot->type = (type == BREAK_READ) ? HW_WATCHPOINT_R : (type == BREAK_WRITE) ? HW_WATCHPOINT_W : HW_WATCHPOINT_RW;
...
int kernel_set_software_breakpoint(uint32_t address, SlotType type) {
    ...
    int read_ret;
    read_ret = ksceKernelCopyFromUserProc(g_target_process.pid, &original_instruction, (void *)address, instruction_size);

    if (read_ret < 0) return read_ret;

    int write_ret;
    write_ret = ksceKernelCopyToUserProcTextDomain(g_target_process.pid, (void *)address, &bkpt_instruction, instruction_size);
...
int kernel_get_callstack(uint32_t *dst, int depth) {
    ...
    while (current_depth < max_depth) {
        ...
        int read_lr_ret;
        read_lr_ret = ksceKernelCopyFromUserProc(g_target_process.pid, &saved_lr, (void *)(current_fp - 4), sizeof(saved_lr));
...
int kernel_single_step(void) {
    ...
    if ((current_registers.cpsr & (1 << 5)) != 0) {
        ...
        if ((thumb_instr & 0xF800) == 0xE000) {
            ...
            // Thumb-2 branch (32-bit instruction) - This condition might need refinement for specific B/BL variants
            uint32_t S = (thumb_instr >> 10) & 1;
            uint32_t imm10 = thumb_instr & 0x3FF;
            uint32_t J1 = (instruction >> 29) & 1;  // Assuming the second half word is already in `instruction`
            uint32_t J2 = (instruction >> 27) & 1;  // Assuming the second half word is already in `instruction`
            uint32_t imm11 = (instruction >> 16) & 0x7FF; // Assuming the second half word is already in `instruction`

            int32_t offset = (S << 24) | (J1 << 23) | (J2 << 22) | (imm10 << 12) | (imm11 << 1);
            if (S) offset |= 0xFE000000; // Sign extend based on S bit

            next_pc = current_pc + 4 + offset; // PC is instruction addr + 4 for Thumb-2
        } else if ((thumb_instr & 0xFF00) == 0x4700) {
            // BX Rm - Branch and Exchange (can't easily predict target PC without register value)
            // Simplification: step over the BX instruction itself. Accurate stepping requires register read.
            next_pc = current_pc + 2;
        } else {
            // Determine if it's a 16-bit or 32-bit Thumb instruction
            // If bits 15-11 are 11101, 11110, or 11111, it's 32-bit.
        ...
        if ((instruction & 0x0F000000) == 0x0A000000) {
            ...
            // Not a branch or BX Rm (BX Rm needs register read for target)
            // Simplification: step over the instruction.
...
int kernel_read_memory(const void *src_addr, void *user_dst, SceSize size) {
    if (src_addr == NULL || user_dst == NULL || size == 0) return -2;
...
int kernel_write_memory(void *dst, const void *user_modification, SceSize memwrite_len) {
    if (!dst || !memwrite_len) return -2;

    void *buf = ksceKernelAllocHeapMemory(g_heap_uid, memwrite_len);
    if (!buf) return -2;

    if (ksceKernelCopyFromUserProc(ksceKernelGetProcessId(), buf, user_modification, memwrite_len) >= 0) 
...
int kernel_write_instruction(void *dst, const void *user_modification, SceSize memwrite_len) {
    if (!dst || !memwrite_len) return -2;
...
int _start(void) {
    if (//module_get_export_func(0x10005, "SceKernelThreadMgr", THREADMGR_NID, SETTHBP_NID, (uintptr_t *)&ksceKernelSetTHBP) < 0 ||
        ...
        module_get_export_func(0x10005, "SceSysmem", SYSMEM_NID, MEMBLKINFO_NID, (uintptr_t *)&sceKernelGetMemBlockInfoByAddr) < 0) {
    ...
    if (heap_uid < 0) return SCE_KERNEL_START_FAILED;
    g_heap_uid = heap_uid;

    kernel_debugger_init();
    gui_init();
    if (rendererv_allocVirtualFB() < 0) gui_destroy();

    guistate.pid = ksceKernelGetProcessId();
    guistate.ui_state = UI_WELCOME;
    guistate.view_state = VIEW_STACK;
    guistate.edit_mode = EDIT_NONE;
    load_hotkeys();

    SceUID thid = ksceKernelCreateThread("pebble", pebble_thread, 0x40, 0x400000, 0, 0, NULL);
    if (thid >= 0) ksceKernelStartThread(thid, 0, NULL);
